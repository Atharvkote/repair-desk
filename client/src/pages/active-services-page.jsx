  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 max-w-6xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm">
        <div>
          <h1 className="text-2xl font-bold text-teal-600 tracking-tight flex items-center gap-2"> <FaFolderOpen className="w-5 h-5 text-teal-600" />{t("activeServices.title")}</h1>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden">
          ...
          <table className="w-full">
            <thead className="bg-slate-50/50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">{t("activeServices.date")}</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">
                            ...
                            <div className="px-6 py-6 space-y-6">
                              <div className="bg-white rounded-xl border border-slate-200/60 p-5 space-y-4">
                                <div className="flex items-center gap-2 text-slate-900 pb-3 border-b border-slate-100">
                                  ...
                                  <h4 className="font-bold text-sm uppercase tracking-wide">{t("activeServices.customerInformation")}</h4>
                                </div>
                              ...
                              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                <div className="bg-white rounded-xl border border-slate-200/60 overflow-hidden">
                                  <div className="flex items-center gap-2 text-slate-900 px-5 py-4 bg-teal-50/30 border-b border-slate-200">
                                    ...
                                    <h4 className="font-bold text-sm uppercase tracking-wide">{t("activeServices.servicesProvided")}</h4>
                                  </div>
                                    ...
                                    {service.services.map((svc) => (
                                      <div key={svc.id} className="px-5 py-3 flex items-center justify-between">
                                        <div className="flex-1">
                                          ...
                                          <div className="text-xs text-slate-500 mt-0.5">{t("activeServices.qty")}: {svc.quantity}</div>
                                        </div>
                                ...
                                <div className="bg-white rounded-xl border border-slate-200/60 overflow-hidden">
                                  <div className="flex items-center gap-2 text-slate-900 px-5 py-4 bg-blue-50/30 border-b border-slate-200">
                                    ...
                                    <h4 className="font-bold text-sm uppercase tracking-wide">{t("activeServices.partsMaterials")}</h4>
                                  </div>
                                  <div className="divide-y divide-slate-100">
                                    {service.parts.map((part) => (
                                      <div key={part.id} className="px-5 py-3 flex items-center justify-between">
                                        <div className="flex-1">
                                          ...
                                          <div className="text-xs text-slate-500 mt-0.5">{t("activeServices.qty")}: {part.quantity}</div>
                                        </div>
                              ...
                              <div className="bg-gradient-to-r from-teal-50 to-blue-50 rounded-xl border border-teal-200/60 p-5 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    <div className="text-xs text-slate-500 mt-0.5">{t("activeServices.dated")}: {service.orderDate}</div>
                                  </div>
                                </div>
        ...
        {filteredServices.length === 0 && (
          <div className="py-12 text-center text-slate-400 font-medium italic">{t("activeServices.noActiveServices")}</div>
        )}
      </div>
    </div>
  )
