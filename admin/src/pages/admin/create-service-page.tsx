    ...
    const loadOrderForEdit = async () => {
      ...
      if (locationState?.editMode && orderId) {
        try {
          ...
          // Set customer
          if (order.customerId) {
            // customerId might be populated object or just ID string
            const customer = typeof order.customerId === 'string' 
              ? null
              : order.customerId
            
            if (customer) {
              const user = await authService.getUser(customer._id || customer.id || customer);
          ...
          if (order.tractor) {
            setTractor({
              name: order.tractor.name || "",
              model: order.tractor.model || "",
            })
          }

          // Set draft order ID
          if (order.id || order._id) {
            setDraftOrderId(order.id || order._id)
          }

          // Transform items to selections
          const allSelections: Selection[] = []
          
          if (order.items && order.items.length > 0) {
            for (const item of order.items) {
              // Fetch catalog item details to get full item info
              let catalogItem: CatalogItem | null = null
              
              try {
                if (item.type === 'service') {
                  catalogItem = await catalogService.getServiceById(item.itemId) as CatalogItem
                } else if (item.type === 'part') {
                  catalogItem = await catalogService.getPartById(item.itemId) as CatalogItem
                }
              } catch (err) {
                console.warn(`Failed to fetch catalog item ${item.itemId}:`, err)
              }
              ...
              if (catalogItem) {
                allSelections.push({
                  item: {
                    ...
                    title: catalogItem.title || catalogItem.name || item.name || "",
                    price: catalogItem.price || item.price || 0,
                    description: catalogItem.description || "",
                    status: catalogItem.status || "AVAILABLE",
                    serviceCode: (catalogItem as any).serviceCode,
                    partCode: (catalogItem as any).partCode,
                  },
                  quantity: item.quantity || 1,
                })
              } else {
                // Fallback: use item data from order if catalog fetch fails
                allSelections.push({
                  item: {
                    _id: item.itemId,
                    id: item.itemId,
          ...
          const apiError = error as ApiError
          console.error("Error loading order for edit:", apiError)
          toast.error(apiError.response?.data?.message || "Failed to load order data")
        } finally {
          setIsLoadingOrder(false)
        }
      } else if (locationState?.editMode && locationState?.serviceData) {
        // Fallback: handle old format if serviceData is passed directly
        const { serviceData } = locationState
        console.warn("Using legacy serviceData format, consider passing orderId instead")

        if (serviceData.customerId) {

          setSelectedUser(serviceData.customerId)
          setUserQuery(serviceData.customerId.name || "");
        }
        ...
        if (serviceData.orderId || serviceData._id) {
          setDraftOrderId(serviceData.orderId || serviceData._id)
        }
